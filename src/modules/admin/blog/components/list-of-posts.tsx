...
import { Table, TableBody, TableCaption, TableCell, TableHead, TableHeader, TableRow } from "@/modules/core/components/ui/table"
...
export const ListOfPosts = ({
    ...
    const columns = Object.keys(posts?.[0] || {}).map(key => key[0].toUpperCase() + key.slice(1));
                    ...
                    {posts.map((post, i) => (
                        ...
                        >
                            {currentColumns?.map((column, i) => {
                                ...
                                const formattedColumn = column.charAt(0).toLowerCase() + column.slice(1);
                                    ...
                                    <TableCell key={post.id + column + i}>
                                        <p
                                            className={cn("line-clamp-2", {
                                                "max-w-40": column.toLowerCase() === "title" || column.toLowerCase() === "content"
                                            })}
                                        >{currentValue?.toString()}</p>
                                    </TableCell>
                                )
                            })}
                            <TableCell>
                                <Icon name="Ellipsis" className="group-hover:hidden" size={16} />
