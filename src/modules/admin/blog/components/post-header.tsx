    ...
    <header className="grid gap-4 items-center md:grid-cols-[1fr_1fr]">
      <div className="space-y-2">
        <Input
          ...
          defaultValue={title}
          onChange={(e) =>
            handlePost({ key: 'title', value: e.currentTarget.value })
          }
          containerClassName={cn({
            'border-destructive/50 has-[:focus]:border-destructive':
              errors?.title,
          })}
        />
      ...
      >
        <div className="space-y-2">
          ...
          <Blockquote title="Note">
            <p>
              Your post will be published on the{' '}
              <b>{formatUpperCase(category)}</b> category
            </p>
          </Blockquote>
        </div>
        <SelectContent>
